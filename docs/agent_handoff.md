...
- Prompt-based CLIP scoring is now an additive path: try `pallavijainpj/SenCLIP` first, keep RemoteCLIP as a separate baseline, and use contrastive scoring `spawn_mean - max(confounder_group_means)` with multi-scale crops for small localized plumes. Keep inference RAM-safe by processing one image at a time with small crop batches. BioCLIP/BiomedCLIP are not recommended for nadir Sentinel-2 imagery.
- Prompt calibration is first-class: benchmark outputs now include raw spawn score, per-confounder group scores, max confounder group/mean, margin, and threshold sweeps (baseline 0.0, best accuracy, best balanced accuracy, best F1).
- `scripts/rs_foundation_probe.py` is the low-RAM linear-probe path for frozen remote-sensing backbones. DINOv2 is the guaranteed fallback; Prithvi/SatMAE/SkySense-style backbones are optional and should fail clearly if deps/model ids are unavailable.
...
- `data/samples/positive/` - positive spawn thumbnails (16 human-verified files). Used for training.
- `data/samples/negative/` - confirmed non-spawn thumbnails (164 files). Used for training.
- `data/samples/training_manifest.json` - canonical training manifest: 16 positives + 164 negatives, with provenance.
- `data/samples/rejected/` - explicitly rejected files (9 files) that are NOT spawn.
...
- `data/final_review/golden_set.json` - final golden set provenance from dual_subspace_contrast review.
- `data/dual_subspace_review/labels.json` - dual-subspace contrast labeling session results.

Current handoff human-reviewed positives (16, from `data/samples/training_manifest.json`):

- 7 Strait of Georgia (2019-2021, verified via dual_subspace_contrast)
- 3 milbanke-sound (2024-03-24, verified via SVM scan)
- 2 nanaimo (2024-03-18, verified via SVM scan)
- 1 dfo-verified-breakwater-island (2024-03-18, Rose-verified)
- 1 dfo-verified-qualicum-beach (2024-03-15, Rose-verified)
- 1 barkley-sound (2024-05-22, promoted via dual_subspace_contrast)
- 1 dfo-verified-fan-island (2024-03-22, promoted via dual_subspace_contrast)

Explicit rejects (9 files in `data/samples/rejected/`):
- big-bay-prince-rupert (2), anderson-point, ucluelet, nootka-sound (2), qualicum, tofino (2)

Do not treat model-ranked candidates, high-confidence score buckets, temporal positives, or generated review pages as human-reviewed positives unless they also appear in an explicit human label file. When promoting labels into training manifests, preserve reviewer/source provenance.
...
Prompt and RS foundation benchmarks:

```bash
source .venv/bin/activate
python scripts/benchmark_prompt_models.py --output-dir data/prompt_benchmarks
python scripts/rs_foundation_probe.py --output-dir data/rs_probe_benchmarks --backbone dinov2
```

Interpretation notes:

- Prompt benchmark baseline metrics use threshold 0.0; calibration tables add best accuracy, best balanced accuracy, and best F1 thresholds.
- RS probe uses frozen embeddings and a small sklearn linear classifier; DINOv2 is the guaranteed fallback when optional Prithvi/SatMAE/SkySense backbones are unavailable.
- Both benchmarks are intentionally low-RAM: one image at a time, no bulk tensor caching.
...
- The positive dataset is now 16 files; still small but human-verified and consistent.
- Single-image DINOv2 models remain the strongest practical detectors (dual-subspace AUROC 0.981, SVM 95.6%, RS probe 0.972).
- Temporal approaches are limited by Sentinel-2 revisit + cloud cover; Landsat 8/9 addition improves coverage 2.4x.
- Environmental matching (sun+tide) with S2+Landsat is the best temporal method currently (AUROC 0.745).
- Prompt-based methods lag behind embedding methods (RemoteCLIP AUROC 0.729).
- SenCLIP dependencies (`transformers`, `huggingface_hub`) need installation to benchmark properly.
- Generated review pages and JSON outputs can become too large for normal Git pushes; upload to Hugging Face.

### Immediate Next Steps
1. Expand Landsat coverage to all 16 positives + scan negatives for better temporal baselines
2. Re-run multi-year SVM scan with finer 0.01° grid spacing
3. Install SenCLIP deps and benchmark against RemoteCLIP
4. Upload image-heavy outputs to Hugging Face, clean local disk
